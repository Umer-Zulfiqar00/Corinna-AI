...
const toChatRole = (role: unknown): "assistant" | "user" | null => {
    if (role === 'OWNER') return 'assistant'
...
export const useChatTime = (creatdAt: Date, roomId: string) => {
    ...
    const onSeenChat = async () => {
        if (chatRoom == roomId && urgent) {
            await onViewReadMessages(roomId)
            setUrgent(false)
        }
    }
    ...
    const onHandleSentMessage = handleSubmit(async (values) => {
        try {
            ...
            if (message) {
                const next = {
                    ...message.message[0],
                    role: toChatRole(message.message[0].role),
                }
